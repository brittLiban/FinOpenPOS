...
export default function CompanySettingsPage() {
  const [platformFeePercent, setPlatformFeePercent] = useState<number>(2.50);
  ...
  const fetchPlatformFee = async () => {
    ...
    try {
      const response = await fetch('/api/admin/platform-fee');
      const data = await response.json();
      setPlatformFeePercent(data.platformFeePercent || 2.50);
    } catch (error) {
      console.error('Error fetching platform fee:', error);
      setMessage('Error fetching platform fee settings');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage(data.error || 'Error updating platform fee');
      }
    } catch (error) {
      console.error('Error updating platform fee:', error);
      setMessage('Error updating platform fee');
    } finally {
      setSaving(false);
    }
  };
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                This is the percentage fee charged on each transaction. Range: 0% - 10%
              </p>
            </div>
            ...
            <div className="bg-muted p-4 rounded-lg">
              ...
              <div className="text-sm space-y-1">
                <div>Sale Amount: $100.00</div>
                <div>Platform Fee ({platformFeePercent}%): ${(100 * platformFeePercent / 100).toFixed(2)}</div>
                <div className="font-medium border-t pt-1">
                  Business Receives: ${(100 - (100 * platformFeePercent / 100)).toFixed(2)}
                </div>
              </div>
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2 text-sm">
              <p>
                <strong>Current Platform Fee:</strong> {platformFeePercent}%
              </p>
              <p>
                <strong>When fees are charged:</strong> Platform fees are automatically deducted from each successful transaction.
              </p>
              <p>
                <strong>Returns:</strong> When customers return items, the platform fee is refunded back to your account.
              </p>
              <p>
                <strong>Processing:</strong> Fees are calculated and processed automatically through Stripe Connect.
              </p>
            </div>
          </CardContent>
        </Card>
